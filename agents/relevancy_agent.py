...
def fact_checking_agent(query, top_responses, retrieved_context):
    ...
    """
    try:
        response1_raw = top_responses[0]["response"] if len(top_responses) > 0 else "Not Found"
        response2_raw = top_responses[1]["response"] if len(top_responses) > 1 else "Not Found"

        # ✅ If response is JSON-encoded, Parse It
        response1 = json.loads(response1_raw) if isinstance(response1_raw, str) and response1_raw.startswith("[") else response1_raw
        response2 = json.loads(response2_raw) if isinstance(response2_raw, str) and response2_raw.startswith("[") else response2_raw

        # ✅ If Parsing Returns a List, Extract First Element
        response1 = response1[0] if isinstance(response1, list) else response1
        response2 = response2[0] if isinstance(response2, list) else response2

    except json.JSONDecodeError:
        print("⚠️ Error: Failed to decode response JSON. Using raw text.")
        response1, response2 = response1_raw, response2_raw
