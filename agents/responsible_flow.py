...
def responsible_agent(summarized_text):

    ai_pipeline = ResponsibleAIPipeline()
    response = summarized_text['summary']
    citations = summarized_text['citations']

    # Step 1: Run Anonymization
    print("anonymizing text")
    anonymized_text = ai_pipeline.anonymize(response)
...
# ✅ Append Citations to Final Processed Text
    formatted_citations = " ".join([f"[{i+1}] {cite}" for i, cite in enumerate(citations)]) if citations else "No citations available."
    final_text_with_citations = f"{final_text}\n\n**References:** {formatted_citations}"

    print("final_text_with_citations", final_text_with_citations)
    return final_text_with_citations
