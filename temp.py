...
if __name__ == "__main__":
    # Take user input (CLI-based for now, API integration later)
    user_query = "Punishment for murder"
    law_type = "criminal_law"
    # run_pipeline(user_query,law_type)
    retrieved_texts = [
    {"text": "Section 302 of the Indian Penal Code (IPC) states that whoever commits murder shall be punished with death, or imprisonment for life, and shall also be liable to a fine."},
    {"text": "In the case of Bachan Singh vs State of Punjab (1980), the Supreme Court ruled that the death penalty should only be applied in the 'rarest of rare' cases."},
    {"text": "Section 304 IPC deals with culpable homicide not amounting to murder, prescribing imprisonment of up to ten years or life imprisonment based on the severity of the offense."}
    ]
    llm_responses = [
        {
            "response_id": "resp_001",
            "response": "According to IPC Section 302, the punishment for murder is life imprisonment or the death penalty. The Supreme Court has ruled that the death penalty should only be given in the 'rarest of rare' cases.",
            "citations": ["Indian Penal Code, Section 302", "Bachan Singh vs State of Punjab, 1980"]
        },
        {
            "response_id": "resp_002",
            "response": "Murder is punishable under Section 302 of IPC, prescribing either the death penalty or life imprisonment. Additionally, Section 304 IPC defines culpable homicide as an offense distinct from murder.",
            "citations": ["Indian Penal Code, Sections 302, 304"]
        }
    ]
    process_evaluation(retrieved_texts,llm_responses)
